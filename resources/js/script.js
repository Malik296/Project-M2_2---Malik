class Deposit {
    ...
    checkFunction() {
        if (this.firstAmount < 0) {
            alert('Не корректная начальная сумма!');
            return false;
        } else if (this.mounthlyAmount < 0) {
            alert('Не корректная сумма ежемесячного пополнения!');
            return false;
...
class Calculator {
    ...
    filteredArray() {
        let filteredArray = this.bankProductArr.filter(bankProduct => bankProduct.currency == this.deposit.typeCurrency);
        filteredArray = filteredArray.filter(bankProduct => (this.deposit.firstAmount >= bankProduct.sumMin && (this.deposit.firstAmount <= bankProduct.sumMax || bankProduct.sumMax == null)));
        filteredArray = filteredArray.filter(bankProduct => (this.deposit.period >= bankProduct.termMin && this.deposit.period <= bankProduct.termMax));
...
class Application {
    constructor() {
        ...
        this.button = document.getElementById("action");
        const self = this;
    ...
    callFunction() {
        ...
        if (deposit.checkFunction()) {
            let calc = new Calculator(deposit);
            calc.initialFunction(res);
